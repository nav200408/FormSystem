...
public class FieldControllerTest {
        ...
        void addFieldToForm_ValidationError() throws Exception {
                ...
                mockMvc.perform(post("/forms/1/fields")
                                ...
                                .andExpect(jsonPath("$.errors.validation.fieldLabel")
                                                .value(MessageConstant.FIELD_LABEL_REQUIRED));
        }
        ...
        @Test
        void reorderFields_Success() throws Exception {
                ReorderFieldRequest request = new ReorderFieldRequest();
                request.setFieldIds(java.util.Arrays.asList(2L, 1L));

                doNothing().when(fieldService).reorderFields(eq(1L), any(ReorderFieldRequest.class));

                mockMvc.perform(put("/forms/1/fields/reorder")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(request)))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.message").value(MessageConstant.UPDATE_SUCCESS));
